 ...
 * 7.进行了优化，使用java8 lambda表达式进行软断言，断言失败让继续运行后续断言代码，增加了脚本的容错性
...
//    @BeforeEach
    ...
    void listDepartment(){
        ...
        assertAll("查询返回值校验",
        ()->assertEquals("09",listResponse.path("errcode").toString()),
        ()->assertEquals(departmentId+"x",listResponse.path("department.id[0]").toString()),
        ()->assertEquals(createName+"x",listResponse.path("department.name[0]").toString()),
        ()->assertEquals(createEnName+"x",listResponse.path("department.name_en[0]").toString())
        );
    }
